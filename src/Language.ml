  ...
  struct
    ...
    *)
    let rec eval env (s, i, o) t =
       match (s, i, o), t  with
         | (s, z::i, o), Read x -> ((State.update x z s), i, o)
         | (s, i, o), Write e -> (s, i, o @ [Expr.eval s e])
         | (s, i, o), Assign (x, e) -> ((State.update x (Expr.eval s e) s), i, o)
         | conf, Seq (t1, t2)  -> eval env (eval env conf t1) t2
         | conf, Skip -> conf
         ...
         | conf, While (expr, st) -> if ((Expr.eval s expr) != 0)
           then eval env conf (Seq (st, While (expr, st)))
           else eval env conf Skip
         | conf, Repeat (st, expr) -> let (s', i', o') = eval env conf st
           in if ((Expr.eval s' expr) == 0)
             then eval env (s', i', o') (Repeat (st, expr))
             else eval env (s', i', o') Skip
         | (s, i, o), Call (name, args_expr) ->
           let (args, local, body) = env#definition name in
           let eval_args = List.map (Expr.eval s) args_expr in
           let scope = State.push_scope s (args @ local) in
           let state = List.fold_left (fun s (x, a) -> State.update x a s) scope (List.combine args eval_args) in
           let (s', i', o') = eval env (state, i, o) body in
    ...
    ostap (
      simple_stmt:
         ...
         | "skip" { Skip } (* Skip *)
         | fn:IDENT "(" args:(!(Util.list0By) [ostap (",")] [Expr.parse]) ")" { Call (fn, args)}; (* Call *)
